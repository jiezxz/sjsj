	...
	<view>
		...
		<view v-if="menu.length" class="menu-container">
			<view class="menu-left">
				<view @click="itemClick(item,index)" v-show="item.enable==1||item.enable===undefined" class="menu-item"
					:class="{'menu-item-select':selectIndex===index}" :key="index" v-for="(item,index) in getMenu()">
					{{item.name}}</view>
