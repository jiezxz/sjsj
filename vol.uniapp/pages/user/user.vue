<template>
	<view style="background: #fbfbfb;">
		...
		<!-- #ifdef MP-WEIXIN -->
		<view v-if="userInfo.showBind" style="margin: 30rpx;">
			<!-- https://uniapp.dcloud.net.cn/component/button.html#button -->
			<button style="font-size: 26rpx;padding: 12rpx;border-radius: 50rpx; color: #fff; background: #4fb1ff;"
				@click="getUserProfile" type="default" class='btn'>绑定微信</button>
		</view>
		<!-- 	#endif -->
	<!-- 	组件已废弃，对应pages.json中tabBar属性 -->
		<!-- 		<vol-tabbar :index="5"></vol-tabbar> -->
	</view>
</template>
		...
		methods: {
			...
			getUserInfo() {
				this.http.post("api/user/getCurrentUserInfo", {}).then(x => {
					...
					this.userInfo.showBind = !x.data.showBind;
